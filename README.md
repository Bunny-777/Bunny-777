
<h1 align="center">Hi 👋, I'm Bunny</h1>
<h3 align="center">Full Stack developer and Programmer</h3>

<img align="right" alt="Coding" width="400" src="https://media4.giphy.com/media/v1.Y2lkPTc5MGI3NjExendvNms2cHIxYnpjbDlqbGJhc21jcmdodm5oaHdueXY5NWFobWY5dCZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/123t0dxx3bQdCE/giphy.gif">

<p align="left"> <img src="https://komarev.com/ghpvc/?username=bunny-777&label=Profile%20views&color=0e75b6&style=flat" alt="bunny-777" /> </p>


I focus on writing clean, maintainable code and designing systems that perform well under real-world constraints.

💼 **Open to freelance and contract opportunities**


---

## 🧠 What I Work On
- 🏗️ Designing scalable full-stack architectures
- 🔌 Building production-ready APIs
- ⚡ Performance optimization & caching
- 🔐 Secure authentication & authorization

---

## 📚 Currently Learning
- Advanced backend communication patterns
- Redis, queues & pub/sub systems
- WebRTC & automated testing

---

## 💻 Tech Stack

### 🎨 Frontend
- React, Next.js, TypeScript
- Tailwind CSS, Component Libraries

### 🧩 Backend
- Node.js, Express, Next.js
- Prisma, WebSockets

### 🗄️ Databases
- PostgreSQL, MongoDB, Redis

### ☁️ Cloud & Deployment
- AWS, Vercel, Render

### 🔐 Authentication
- NextAuth
- SupaBase
- Custom SMTP auth

### 🧠 State Management
- Zustand, Recoil, Redux

---

## 🚀 Featured Projects
- **CrashGuard** 🔗 [Repo](https://github.com/Bunny-777/Crash-Guard) · 🌐 [Live](https://github.com/Bunny-777/Crash-Guard) 
   Developed a crash detection Android application capable of identifying accidents within 10 seconds, helping save thousands of lives by triggering emergency alerts.

- **Stalker**  🔗 [Repo](https://github.com/Bunny-777/Stalker) 🌐 [Live](https://drive.google.com/file/d/1gkO9ffzk55Qs8-8Wkkv8mwEkxqqMoyII/view?usp=sharing) 
  Built a cross-platform mobile application for tracking a user’s LeetCode activity 24x7 and providing
immediate response through Telegram push notifications.

- **Event API** 🔗[Repo](https://github.com/Bunny-777/Event-api) · 🌐 [Live](https://event-api-89d4.onrender.com/)
   Engineered an event management API with 20+ endpoints and PostgreSQL-optimized queries, enabling
users to create events, register/cancel participation, and analyze event statistics with an average
latency of less than 150ms

---

## 🌐 Connect With Me
[![LinkedIn](https://img.shields.io/badge/LinkedIn-bunny777-blue?logo=linkedin)](https://www.linkedin.com/in/bunny777/)
[![LeetCode](https://img.shields.io/badge/LeetCode-bunny77-orange?logo=leetcode)](https://leetcode.com/u/bunny77/) 
[![Codeforces](https://img.shields.io/badge/Codeforces-bunny777-red?logo=codeforces)](https://codeforces.com/profile/bunny777) 
[![CodeChef](https://img.shields.io/badge/CodeChef-bunny777-brown?logo=codechef)](https://www.codechef.com/users/bunny777) 
[![X](https://img.shields.io/badge/X-@bunny__777-black?logo=x)](https://x.com/bunny__777) 



---

## 📊 GitHub Snapshot
![](https://github-readme-stats.vercel.app/api/top-langs/?username=lakshay-tiwari&theme=dark&hide_border=false&layout=compact)

[![](https://visitcount.itsvg.in/api?id=lakshay-tiwari&icon=0&color=0)](https://visitcount.itsvg.in)










![snake gif](https://github.com/Bunny-777/bunny-777/blob/output/github-snake-dark.svg)








